// Vercel serverless entrypoint — API routes only (no static file serving)
// Static files are served directly by Vercel from dist/
...
const supabase = supabaseUrl && supabaseKey
  ? createClient(supabaseUrl, supabaseKey, { auth: { autoRefreshToken: false, persistSession: false, detectSessionInUrl: false } })
  : null;
...
function normalizeUser(user) {
  return { ...user, plan: user.plan || "free", planName: user.planName || "Explorer", planStatus: user.planStatus || "free", role: user.role || "user", profile: user.profile || defaultProfile(user.country), is_paid: !!(user.is_paid || (user.plan && user.plan !== "free")), paid_at: user.paid_at || null };
}
...
app.get("/api/config", (_req, res) => {
  res.json({
    supabase: { configured: !!supabase, url: supabaseUrl || "", anonKey: supabaseKey || "" },
    pricingPlans: [],
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    const user = normalizeUser({
      id: crypto.randomUUID(), name, email, passwordHash: createPasswordHash(password), country,
      plan: "free", planName: "Explorer", planStatus: "free",
      profile: defaultProfile(country), createdAt: nowIso()
    });
    db.users.push(user);
...
app.post("/api/auth/forgot-password", async (req, res) => {
  try {
    ...
    await saveDb(db);
    return res.json({ message: "If that email is registered, a reset link has been sent.", token }); // token included for dev
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
});
...
app.get("/api/location", async (req, res) => {
  try {
    ...
    if (data.country_name) {
      return res.json({ country: data.country_name, detected: true });
    }
